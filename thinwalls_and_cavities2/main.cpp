...
int main(int argc, char *argv[])
{
    ...
    if (argc > 1)
    {
        ...
        // Input Filename
        if (vm.count(map_param)) {
            map_filename = vm[map_param].as<std::string>();
            has_map = true;
        }
        ...
        // Epsilon threshold
        if (vm.count(thresh_perc_param))
        {
            if (vm[thresh_perc_param].as<double>() > 0.0)
                thresh_perc = vm[thresh_perc_param].as<double>();
            else
            {
                std::cerr << "Max Epsilon must be positive." << std::endl;
                exit(1);
        ...
        // Compute all epsilons
        if (vm.count(all_param))
        {
            compute_all = true;

            if (vm.count(store_all_spheres))
                store_intermediate_spheres = true;
        }
        else
        if (max_epsilon == 0.0)
        {
            std::cerr << "At least one inbtween --" << max_epsilon_param << " and --" << all_param << " must be used." << std::endl;
            //exit(1);
        }
    ...
    else
    {
        ...
        filename            = "/media/daniela/Shared/Devel/data/Tet_boundaries/187.wrl.tet.off";    ////// TO BE REMOVED
        //filename            = "/home/daniela/Scrivania/cylinder_cut.off";    ////// TO BE REMOVED
        compute_all         = true;
        do_it_parallel      = 1; ////// TO BE REMOVED
        //compute_sdf         = true;
        //max_epsilon         = 0.02;    ////// TO BE REMOVED
        //search_thinwalls    = true;
        search_cavities     = true;
        //repair              = true;
        //store_intermediate_spheres = true;
    ...
    if (max_epsilon > 0.0)
        std::cout << "[Epsilon Threshold]: " << max_epsilon << std::endl;

    if (compute_all == true)
        std::cout << "Epsilon will be evaluated for each triangle." << std::endl;
    ...
    if (thresh_perc != FLT_MAX)
    {
        std::cout << "Thickness threshold : " << thresh_perc << " * bb.diag = " ;

        max_epsilon = thresh_perc * eps3D.tin.bbox().diag();

        std::cout << max_epsilon << std::endl;
    }

    eps3D.set_max_epsilon(max_epsilon);
